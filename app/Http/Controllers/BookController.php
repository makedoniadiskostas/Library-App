...
class BookController extends Controller
{
    ...
    public function showEditScreen(Book $book){ // Εφοσον το $post εδω ταιριαζει με το dynamic μερος του edit url, η Laravel αυτοματα θα κανει τον link με το post στο database
        ...
        if(!auth()->check()){
            return redirect('/login')->with('error', 'You must log in first to edit a book.');
        }
    ...
    public function deleteBook(Book $book) {
        ...
        if(!auth()->check()){
            return redirect('/login')->with('error', 'You must log in first to delete a book.');
        }
        elseif (auth()->user()->id === $book['user_id']){ // Ενας user μπορει να κανει delete ενα post μονο αν ειναι δικο του αλλιως γινεται redirect στο home
            $book->delete();
            return redirect('/');
        }
        return redirect('/')->with('error', 'You can not edit or delete a book that was not inserted by you!');;
    ...
    public function categoryFilter(Request $request) {
        ...
        if ($request->filled('category')) { // Ελέγχει αν υπάρχει parameter category στο URL.
